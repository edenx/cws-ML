...
#      for X, Y, l in prange(product(X_array, Y_array, np.array([l]))):
#           ker += [ker_Gauss2d(X, Y, l)]

#      # ker = np.array(list(starmap(ker_Gauss2d, product(X_array, Y_array, np.array([l])))))
#      print("dimension of kernel ", np.array(ker).reshape(n,m).shape)

#      return np.array(ker).reshape(n, m)

# Q1 a) Nearest Mean with Image Classification ---------------------------------------------
def ker_Gauss3d(X_arr, Y_arr, l):
     """Find the kernel averaged over Gaussian kernel on patches between images in class 
     X and Y
     -----------
     Input
     X_arr, Y_arr: n/m by N by r 3d array; each row storing one patched image
     l: positive scalar; sigma, hyperparameter in Gaussian kernel
     -----------
     Output
     n by m kernel matrix between two classes of images
     """
     ...
     N = X_arr.shape[1]
     print("number of patches is ", N, "\n")

     X_arr=np.swapaxes(X_arr, 0, 1)
     Y_arr=np.swapaxes(Y_arr, 0, 1)

     tmp = np.swapaxes(np.tile(np.sum(X_arr**2, axis=2), (m,1,1)), 0, 1)
     print("dimension of pairwise distances", np.sum(X_arr**2, axis=2).shape, "\n")
     print("dimension of pairwise distances", tmp.shape, "\n")
     tmp = np.swapaxes(tmp, 1, 2) #transpose
     tmp += np.swapaxes(np.tile(np.sum(Y_arr**2, axis=2), (n,1,1)), 0, 1)
     tmp -= 2 * X_arr @ np.swapaxes(Y_arr, 1, 2)
     tmp = np.exp(-tmp/l**2)
     # print("dimension of pairwise distances", tmp.shape, "\n")

     # print("kernel shape", ker, '\n')
     # print("entries<0.5", np.sum(ker<0.5))

     return np.mean(tmp, axis=0) 

def ker_Poly3d(X_arr, Y_arr, c):
     """Find the kernel averaged over Polynomial kernel on patches between images in class 
     X and Y
     -----------
     Input
     X_arr, Y_arr: n/m by N by r 3d array; each row storing one patched image
     c: non-negative scalar; c, hyperparameter in Polynomial kernel
     -----------
     Output
     n by m kernel matrix between two classes of images
     """
     ...
     tmp = (X_arr @ np.swapaxes(Y_arr, 1, 2) + c)**2
     # print("dimension of kernel", tmp.shape, '\n')
...
def pad_patches(imag, p=3):
     ...
     ind_hstack = (shape_out[0], 1, p, p)
     ind_vstack = (1,shape_out[1]+2,p,p)

     pdd_patches = np.hstack((np.zeros(ind_hstack), renorm_patches, np.zeros(ind_hstack)))
     #+2 accounting for the extra dim
     pdd_patches = np.vstack((np.zeros(ind_vstack), pdd_patches, np.zeros(ind_vstack))) 
...
if __name__=='__main__':
     ...
     cla = [0]
     cla_index = np.concatenate([np.where(target == i)[0] for i in cla])
     arr_dat = lfw_people.images[cla_index]
     # scale the original image
     arr_dat = (arr_dat - np.mean(arr_dat, axis=0))/np.std(arr_dat, axis=0)
     ...
     # t0 = time.time()
     # temp = Wrapper_dprocess(arr_dat, p=3)
     # t1 = time.time()
     # print("time taken to patch no padding ", t1-t0)

     # t0 = time.time()
     # temp = Wrapper_pad_patches(arr_dat, p=3)
     # t1 = time.time()
     # print("time taken to patch with padding ", t1-t0)
     ...
     # t0 = time.time()
     # ker = ker_Imag(dat_train, dat_train, 2)
     # t1 = time.time()
     # print("time for one hyperparameter", t1-t0)

     # ar_ker = ker_Gauss2d(dat_test, dat_test, 1)
     # print("numpy function ", ar_ker)
     # from sklearn.metrics.pairwise import rbf_kernel
     # sk_ker = rbf_kernel(dat_test, dat_test, 1)
     # print("differences", ar_ker - sk_ker)
     # print("differences", np.any((ar_ker - sk_ker)>10**(-7)))

     # choose two classes -----------------------------------------------------------------------------------------
     cla = [0,1]; test_ratio = 0.25
     # kernel_dic = {'Gaussian kernel': ker_Gauss3d
     ...
     t0 = time.time()
     ker = ker_Gauss3d(dat_test, dat_test, 2) # 3d array
     t1 = time.time()
     print("time for one hyperparameter", t1-t0)

     print("vectorised", ker)
     ...
     # # Wrapper_Plot(kernel_dic, hyper_dic, dat_train, dat_test, lab_train, lab_test, p)
     # t0 = time.time()
     # NearestMean(ker_Gauss3d, 2, dat_train, dat_test, lab_train, lab_test)
     # t1 = time.time()
     # print("total time of running", t1-t0)

     # # change of distance to mean  -----------------------------------------------------------------------------------
     # cla_ = [13] # 41 images
     # kernel_dic_ = {'Gaussian kernel': ker_Gauss3d, 'Polynomial kernel': ker_Poly3d}
     # hyper_dic_ = {'$\sigma$': np.linspace(0.1,5,num=40), 'c': np.linspace(0,5,num=40)}
     
     # # patch length
     # p_ = 3
     # proc_dat_ = Wrapper_Dat(cla_, lfw_people, p_)
     # Wrapper_HeatMap(kernel_dic_, hyper_dic_, proc_dat_, p_)
